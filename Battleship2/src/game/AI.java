...
import game.GameEngine.BombStatus;
...
public class AI extends Player {
	 ...
	 * @param battlefield
	 */
	public AI(Difficulty difficulty, Battlefield opponent, ZoneListener listener) {
		super("ai", listener);
	 ...
	 */
	public int[] attack(BombStatus lastShot) {
		...
		case NORMAL: 
			...
			break;
		case HARD: 
			if (((new Random()).nextInt(2) > 0) && lastShot != BombStatus.HIT)
				target = insaneAttack();
			else
				target = normalAttack(lastShot);
			break;
		...
		// Retain these attack coordinates for reference next turn public enum Difficulty {TRAINING, EASY, NORMAL, HARD, INSANE}
		lastAttack = target;
	 ...
	 * @return coordinates to attack
	 */
	private int[] normalAttack(BombStatus lastShot) {
		int[] target;
		if (lastShot == BombStatus.HIT) {
		...
		} else if (lastShot == BombStatus.MISS && neighbours.size() > 0) {
			Iterator<int[]> iter = neighbours.iterator();
			target = iter.next();
			iter.remove();
		} else 
	...
	private void cheatList()
	{
		for (int y = 0; y < 10; y++) {	
			for (int x = 0; x < 10; x++) {
				if (opponent.hasShip(x,y))
					cheat.add(new int[]{x,y});
			}
		}
